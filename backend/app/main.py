    ...
    try:
        ...
        if not leo:
            ...
            school_cp = SafeZone(
                ...
                radius_meters=100.0,
                address="Delhi Public School Gate 1, New Delhi",
                is_active=True,
                alert_on_exit=True,
            )
            db.add(school_cp)
        ...
        # Ensure existing dev-band-leo-1 has nfc_tag_id if null
        leo_band = db.query(Device).filter(Device.id == "dev-band-leo-1").first()
        if leo_band and not leo_band.nfc_tag_id:
            leo_band.nfc_tag_id = "NV-NFC-001"
            db.commit()

        # Seed Caregiver NFC if not present
        if db.query(CaregiverNFC).count() == 0:
            sarah_nfc = CaregiverNFC(
                id="cgnfc-sarah-leo-1",
                caregiver_id="user-verified-sarah",
                child_id="child-leo-1",
